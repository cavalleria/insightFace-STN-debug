...
class ParallModule(BaseModule):
    ...
    def get_export_params(self):
        ...
        for _module in self._arcface_modules:
            fc_g, fc_x = _module.get_params()
            fc_w.update(fc_g)
        for key in fc_w.keys():
            fc_w[key] = fc_w[key].asnumpy()
        #  with open('models/imp-fcw_'+str(self._epoch)+'.pkl', 'wb') as f:
        #      pickle.dump(fc_w, f)
        for i in range(len(self._context)):
            with open('models/sec-fcw_{}_{}.pkl'.format(str(self._epoch), str(i)), 'wb') as f:
                tmp_name = 'fc7_{}_weight'.format(str(i))
                if fc_w.has_key(tmp_name):
                    pickle.dump((tmp_name, fc_w[tmp_name]), f)
        return g, x
	...
	#  print('load weight file done')
        ...
        for _module in self._arcface_modules:
          ...
          #  file_name = '/data/insightface/recognition/models/fc_weights/0102/sec-fcw_55_{}.pkl'.format(str(idx))
          #  tmp_fcw = pickle.load(open(file_name, 'rb'))
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            while not end_of_batch:
                ...
                loss = self.forward_backward(data_batch)
                self.update()
                ...
                fre_loss = 100
                if nbatch % fre_loss == 0:
                    print('nbatch: %7d batch loss : %3f  lr: %f'%(nbatch, loss.asscalar()/self._batch_size, self._curr_module._optimizer.lr))
                nbatch += 1
